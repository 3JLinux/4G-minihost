...
typedef struct _hwgg
{
	...
	u_char ubaEndAddr[2];	2bytes			//???????EndAddr??

	u_char ubSrcLayer;		1byte			//???SrcLayer??
	u_char ubEndLayer;		1byte			//????EndLayer??
...
typedef struct _hwgg
{
	...
	u_char ubaEndAddr[2];				//???????EndAddr??

	u_char ubSrcLayer;					//???SrcLayer??
	u_char ubEndLayer;					//????EndLayer?? or device type
